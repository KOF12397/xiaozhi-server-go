...
func NewConnectionHandler(
	...
	// 修复：注册Mattermost函数（包含完整的Tool结构体定义）
	if handler.functionRegister != nil {
		// 创建正确的 Tool 结构体
		mattermostTool := openai.Tool{
			Type: "function",
		...
		if err := handler.functionRegister.RegisterFunction("send_to_mattermost", mattermostTool); err != nil {
			logger.Error("注册Mattermost函数失败: %v", err)
		} else {
